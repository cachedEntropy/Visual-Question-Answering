...
def batch_accuracy(predicted, true):
    ...
    _, true_index = true.max(dim=1, keepdim=True)

    # print('CHECKING PREDICTIONSSSSS')
    # print(true_index, predicted_index)
    count_correct = torch.nonzero(predicted_index==true_index).size(0)
    count_correct = float(count_correct)/config.batch_size
    # print(count_correct)
    '''
    ...
    Finally, we can combine all cases together with:
        min(agreeing * 0.3, 1)
    '''
    return torch.FloatTensor([count_correct])
